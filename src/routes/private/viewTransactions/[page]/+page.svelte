        ...
        {#if transactionList.length > 0}
            <ul class="list-none space-y-4">
                {#each transactionList as transaction}
                    <li class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg relative hover:bg-gray-100 dark:hover:bg-gray-700 transition-all flex justify-between items-center">
                        <div class="flex flex-col">
                            <p class={`text-xl font-bold ${transaction.type === 1 ? 'text-red-500' : 'text-green-500'}`}>
                        ...
                        <form method="POST" action = '?/deleteTransaction'>
                            ...
                            <button type="submit" class="p-2 bg-red-500 hover:bg-red-600 rounded-full text-white transition-all cursor-pointer">
